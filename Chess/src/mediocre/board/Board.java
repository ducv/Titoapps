...
public class Board implements Definitions {
	 ...
	 * time and should not be nescessary
	 * 
	 ...
	 *            move
	 */
	public synchronized void makeMove(int move) {
		...
		captureHistory[historyIndex] = boardArray[Move.toIndex(move)]; // If
		// the
		// move
		// is an
		// en
		...
		if (enPassant != -1)
			zobristKey ^= Zobrist.EN_PASSANT[enPassant]; // Unmake the
		// enpassant square
		// that is on the
		// board now, and
		// replace it below
	 ...
	 * param handicrap
	 */
	public final void setupStart(char handicrap) {
		/*
		 * this.history = new int[4096]; this.captureHistory = new int[4096];
		 * this.zobristHistory = new long[4096]; this.pawnZobristHistory = new
		 * long[4096]; this.zobristKey = 0; this.pawnZobristKey = 0;
		 */
		switch (handicrap) {
		case '0':
		...
		case 'p':
		case 'P':
			inputFen("rnbqkbnr/pppppppp/8/8/8/8/PP0PPPPP/RNBQKBNR w KQkq - 0 1");
			break;
		case 'n':
		case 'N':
			inputFen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/R0BQKBNR w KQkq - 0 1");
			break;
		case 'r':
		case 'R':
			inputFen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/0NBQKBNR w KQkq - 0 1");
			break;
		case 'q':
		case 'Q':
			inputFen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNB0KBNR w KQkq - 0 1");
			break;
		default:
			inputFen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
			break;
	...
	public final String getFen() {
		...
		else // An en passant is available
		{
			...
			case 5:
				...
				break;
			default:
				fen_string += ""; // "Error in ep square"; // Since en passants
									// only
				// can occur
		...
		while (!fenFinished && i < trimmedFen.length()) {
			...
			case 4: // Half-moves (50 move rule) and full moves
			{
				...
				if (" ".equals(trimmedFen.substring(i + 1, i + 2))) {
					movesFifty = Integer.parseInt(currentChar.trim());
				}
				...
				// there are two numbers and then we're done with half-moves.
					...
					movesFifty = Integer.parseInt(trimmedFen
							.substring(i, i + 2).trim());
					i++;
				}
				i += 2;
				movesFull = Integer.parseInt(trimmedFen.substring(i).trim());
	 ...
	 */
	public final int gen_allLegalMoves(int[] moves, int startIndex) {
		int nMoves = 0;
	...
	public final int distance(int squareA, int squareB) {
		int distance = Math.max(Math.abs(row(squareA) - row(squareB)), Math
				.abs(rank(squareA) - rank(squareB)));

		return distance;
	} // END distance()

	public final int gen_noncaps(int[] moves, int startIndex) {
		...
		if (toMove == WHITE_TO_MOVE) {
				...
				if (pieceType == EMPTY_SQUARE) // Pawn can move forward
				{
					if (rank(to) == 7) // Reached the last rank add promotions
					{
						...
						moves[moveIndex++] = Move.createMove(W_PAWN, from, to,
								0, PROMOTION_ROOK, 0);
						moves[moveIndex++] = Move.createMove(W_PAWN, from, to,
								0, PROMOTION_BISHOP, 0);
						moves[moveIndex++] = Move.createMove(W_PAWN, from, to,
								0, PROMOTION_KNIGHT, 0);
					} else // Ordinary
					{
						moves[moveIndex++] = Move.createMove(W_PAWN, from, to,
								0, ORDINARY_MOVE, 0);
						...
						// can move two squares as well
						{
							...
							// we can add it
							{
								moves[moveIndex++] = Move.createMove(W_PAWN,
										from, to, 0, ORDINARY_MOVE, 0);
							}
						}
					}
			...
			// Rooks
			for (int i = 0; i < w_rooks.count; i++) {
				moveIndex += gen_noncaps_delta(w_rooks.pieces[i], rook_delta,
						4, true, moves, moveIndex);
			}
			// Queen
			for (int i = 0; i < w_queens.count; i++) {
				moveIndex += gen_noncaps_delta(w_queens.pieces[i], queen_delta,
						8, true, moves, moveIndex);
			}
			// King
		...
		} else // Black to move
		{
				...
				if (pieceType == EMPTY_SQUARE) {
					...
					} else // Ordinary capture
					{
						...
						// can move two squares as well
						{
							...
							// we can add it
							{
								moves[moveIndex++] = Move.createMove(B_PAWN,
										from, to, 0, ORDINARY_MOVE, 0);
							}
						}
					}
			...
			// Rooks
			for (int i = 0; i < b_rooks.count; i++) {
				moveIndex += gen_noncaps_delta(b_rooks.pieces[i], rook_delta,
						4, true, moves, moveIndex);
			}
			// Queen
			for (int i = 0; i < b_queens.count; i++) {
				moveIndex += gen_noncaps_delta(b_queens.pieces[i], queen_delta,
						8, true, moves, moveIndex);
			}
			// King
	 ...
	 */
	public final int gen_caps(int[] moves, int startIndex) {
		int moveIndex = startIndex;
		int from, to;
		...
		if (toMove == WHITE_TO_MOVE) {
					...
					if (pieceType < 0) // Black piece
					{
						...
						// capture so add promotions
						{
							moves[moveIndex++] = Move.createMove(W_PAWN, from,
									to, pieceType, PROMOTION_QUEEN, 0);

							moves[moveIndex++] = Move.createMove(W_PAWN, from,
									to, pieceType, PROMOTION_ROOK, 0);

							moves[moveIndex++] = Move.createMove(W_PAWN, from,
									to, pieceType, PROMOTION_BISHOP, 0);

							moves[moveIndex++] = Move.createMove(W_PAWN, from,
									to, pieceType, PROMOTION_KNIGHT, 0);

						} else // Ordinary capture
						{
							moves[moveIndex++] = Move.createMove(W_PAWN, from,
									to, pieceType, ORDINARY_MOVE, 0);
			...
			// Knights
			for (int i = 0; i < w_knights.count; i++) {
				moveIndex += gen_caps_delta(w_knights.pieces[i], knight_delta,
						8, false, moves, moveIndex);
			}
			// Bishops
			for (int i = 0; i < w_bishops.count; i++) {
				moveIndex += gen_caps_delta(w_bishops.pieces[i], bishop_delta,
						4, true, moves, moveIndex);
			}
			// Rooks
			for (int i = 0; i < w_rooks.count; i++) {
				moveIndex += gen_caps_delta(w_rooks.pieces[i], rook_delta, 4,
						true, moves, moveIndex);
			}
			// Queen
			for (int i = 0; i < w_queens.count; i++) {
				moveIndex += gen_caps_delta(w_queens.pieces[i], queen_delta, 8,
						true, moves, moveIndex);
			}
			// King
			moveIndex += gen_caps_delta(w_king.pieces[0], king_delta, 8, false,
					moves, moveIndex);
		} else // Black to move
		{
			...
			// Knights
			for (int i = 0; i < b_knights.count; i++) {
				moveIndex += gen_caps_delta(b_knights.pieces[i], knight_delta,
						8, false, moves, moveIndex);
			}
			// Bishops
			for (int i = 0; i < b_bishops.count; i++) {
				moveIndex += gen_caps_delta(b_bishops.pieces[i], bishop_delta,
						4, true, moves, moveIndex);
			}
			// Rooks
			for (int i = 0; i < b_rooks.count; i++) {
				moveIndex += gen_caps_delta(b_rooks.pieces[i], rook_delta, 4,
						true, moves, moveIndex);
			}
			// Queen
			for (int i = 0; i < b_queens.count; i++) {
				moveIndex += gen_caps_delta(b_queens.pieces[i], queen_delta, 8,
						true, moves, moveIndex);
			}
			// King
			moveIndex += gen_caps_delta(b_king.pieces[0], king_delta, 8, false,
					moves, moveIndex);
		}
	 ...
	 */
	private final int gen_noncaps_delta(int index, int[] delta, int nDelta,
			boolean sliding, int[] moves, int startIndex) {
		int moveIndex = startIndex;
		...
		for (int i = 0; i < nDelta; i++) // Loop through the 8 possible deltas
		{
			...
			while ((deltaIndex & 0x88) == 0
					&& boardArray[deltaIndex] == EMPTY_SQUARE) {
				moves[moveIndex++] = Move.createMove(boardArray[index], index,
	 ...
	 * @return totalMovesAdded The number of moves that were added to the array
	 */
	private final int gen_caps_delta(int index, int[] delta, int nDelta,
			boolean sliding, int[] moves, int startIndex) {
		int moveIndex = startIndex;
		// Record the board's en passant square, white/black castling rights and
		// half-moves
		for (int i = 0; i < nDelta; i++) // Loop through the 8 possible deltas
		{
			...
			while ((deltaIndex & 0x88) == 0) {
				if (boardArray[deltaIndex] == EMPTY_SQUARE) {
					...
					moves[moveIndex++] = Move.createMove(boardArray[index],
							index, deltaIndex, boardArray[deltaIndex],
							ORDINARY_MOVE, 0);
